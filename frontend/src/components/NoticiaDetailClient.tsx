...
export default function NoticiaDetailClient({ slug }: NoticiaDetailClientProps) {
  ...
  if (!noticia) {
    return (
      <div className="min-h-screen bg-[#F4F4F2] flex flex-col items-center justify-center font-sans gap-4">
        <p className="text-xs font-bold uppercase tracking-widest text-neutral-400">Matéria não encontrada</p>
        <Link href="/noticias/" className="px-6 py-3 bg-neutral-950 text-white text-[9px] font-bold uppercase tracking-widest rounded-xl transition-all hover:bg-neutral-800 shadow-2xs">
  ...
  return (
    <main className="min-h-screen bg-[#F4F4F2] pb-32 text-neutral-900 selection:bg-neutral-900 selection:text-white font-sans antialiased">
      ...
      <div className="w-full border-b border-neutral-200/60 mb-8 print:hidden">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <nav className="flex items-center gap-3 text-[9px] font-bold uppercase tracking-[0.2em] text-neutral-400">
            ...
            <span>/</span>
            <Link href="/noticias/" className="hover:text-neutral-950 transition-colors">Notícias</Link>
            <span>/</span>
            <span className="text-neutral-950 font-bold truncate max-w-[200px] md:max-w-md">
              {noticia.titulo}
            </span>
          </nav>
        </div>
      ...
      <article className="max-w-4xl mx-auto px-6 pt-8 md:pt-12">
        
        {/* CABEÇALHO DA MATÉRIA */}
        <header className="mb-10 border-b border-neutral-300 pb-8 flex flex-col items-center text-center w-full">
          
          <div className="mb-4 select-none">
            {isGeral ? (
              <span className="text-neutral-500 bg-neutral-200/60 text-[9px] font-bold uppercase tracking-widest px-3 py-1 rounded-full">
                Informativo Geral
              </span>
            ) : (
              <span className="text-white bg-neutral-950 text-[9px] font-bold uppercase tracking-widest px-3 py-1 rounded-full">
                Campus {(noticia as any).campus_display}
              </span>
            )}
          </div>

          <h1 className="text-2xl md:text-4xl font-bold tracking-tight text-neutral-950 uppercase mb-6 max-w-3xl leading-snug">
            {noticia.titulo}
          </h1>

          <p className="text-xs md:text-sm text-neutral-500 font-light leading-relaxed max-w-2xl mb-6">
          ...
          <div className="flex items-center justify-center gap-4 text-[9px] font-bold uppercase tracking-[0.15em] text-neutral-400">
            <span className="text-neutral-950 font-bold">Por {noticia.autor || 'Redação DCE'}</span>
        ...
        {listaTags.length > 0 && (
          <div className="w-full flex flex-wrap items-center gap-2 mb-12 select-none print:hidden">
            <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-neutral-400 mr-1">Assuntos:</span>
            {listaTags.map((tag: string, index: number) => (
              <span key={index} className="bg-white text-neutral-600 border border-neutral-200/60 text-[9px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full shadow-3xs">
                {tag}
              </span>
            ))}
          </div>
        ...
        <footer className="pt-8 border-t border-neutral-300 flex justify-between items-center print:hidden">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-neutral-950 flex items-center justify-center text-[10px] font-bold text-white uppercase tracking-wider select-none">
                {(noticia.autor || 'DC').substring(0,2).toUpperCase()}
            </div>
            <p className="text-[10px] font-bold text-neutral-950 uppercase tracking-[0.15em]">DCE UFVJM</p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 border-l border-neutral-300 pl-3">
                <a 
                  ...
                  className="p-2.5 bg-white text-neutral-900 hover:bg-neutral-950 hover:text-white transition-all rounded-xl border border-neutral-200/60 shadow-2xs"
                ...
                <Link href="/noticias/" className="ml-1 px-5 py-2.5 bg-white hover:bg-neutral-950 border border-neutral-200/60 hover:border-neutral-950 text-neutral-900 hover:text-white text-[10px] font-bold uppercase tracking-wider rounded-xl transition-all shadow-2xs">
                  Voltar
                </Link>
            </div>
          </div>

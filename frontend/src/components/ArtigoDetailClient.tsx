...
import { ChevronRight, BookOpen, Share2 } from 'lucide-react';
import PrintButton from '@/components/PrintButton';
import { useRouter } from 'next/navigation';

interface ArtigoDetailClientProps {
  slug: string;
}

export default function ArtigoDetailClient({ slug }: ArtigoDetailClientProps) {
  ...
  useEffect(() => {
    ...
    if ((!realSlug || realSlug === 'detalhe' || realSlug === 'fallback') && typeof window !== 'undefined') {
      ...
      if (pathParts[0] === 'artigos' && pathParts[1]) {
    ...
    async function fetchDetail() {
      try {
        const res = await api.get<Artigo>(`artigos/${realSlug}/`);
        setArtigo(res.data);
      } catch (error) {
        console.error("Erro ao buscar conteúdo dinâmico do artigo:", error);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F8F9FA] flex flex-col gap-4 items-center justify-center font-sans">
        ...
        <span className="text-xs font-bold uppercase tracking-[0.3em] text-neutral-500">Abrindo Artigo...</span>
      </div>
    );
  }
  ...
  if (!artigo) {
    return (
      <div className="min-h-screen bg-[#F8F9FA] flex flex-col items-center justify-center font-sans gap-4">
        <p className="text-xs font-bold uppercase tracking-widest text-neutral-400">Artigo não encontrado</p>
        <Link href="/artigos" className="px-6 py-3 bg-neutral-950 text-white text-[9px] font-bold uppercase tracking-widest rounded-xl transition-all hover:bg-neutral-800 shadow-sm">
          Voltar para Artigos
        </Link>
      </div>
    );
  ...
  const shareUrl = typeof window !== 'undefined' ? window.location.href : `http://dce.ufvjm.edu.br/artigos/${artigo.slug}/`;
  const shareText = encodeURIComponent(`Portal do DCE - Artigo: ${artigo.titulo}`);
      ...
      <div className="w-full border-b border-neutral-200/60 mb-8 bg-white/50 backdrop-blur-md sticky top-0 z-30 print:hidden">
        <div className="max-w-[1440px] mx-auto px-4 md:px-6 py-4">
          <nav className="flex items-center gap-3 text-[9px] font-black uppercase tracking-[0.2em] text-neutral-400">
            ...
            <Link href="/artigos" className="hover:text-[#0073B7] transition-colors">Artigos</Link>
            <span className="text-neutral-300">/</span>
      ...
      <article className="max-w-4xl mx-auto px-4 md:px-6 pt-6 md:pt-10">
        ...
        <header className="mb-10 border-b border-neutral-200 pb-8 flex flex-col items-center text-center w-full">
          ...
          <div className="mb-4 select-none">
            <span className="bg-[#0073B7]/5 text-[#0073B7] text-[8px] px-3 py-1.5 font-black uppercase tracking-[0.15em] rounded-full border border-[#0073B7]/10 flex items-center gap-2">
              <BookOpen size={10} />
              Artigo de Opinião
            </span>
          </div>
          ...
          <p className="text-xs md:text-sm text-neutral-500 font-normal opacity-90 leading-relaxed max-w-2xl mb-6 italic">
            "{artigo.resumo}"
          </p>
          
          <div className="flex items-center justify-center gap-4 text-[9px] font-black uppercase tracking-[0.2em] text-neutral-400">
            <span className="text-neutral-950">Por {artigo.autor}</span>
            <span className="w-1 h-1 bg-neutral-300 rounded-full"></span>
            <span>Publicado: {new Date(artigo.data_publicacao).toLocaleDateString('pt-BR')}</span>
          </div>
        </header>

        {/* CORPO DO ARTIGO PADRONIZADO EM SANS-SERIF */}
        <div 
        ...
        <footer className="pt-8 border-t border-neutral-200 flex justify-between items-center print:hidden">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-neutral-950 flex items-center justify-center text-[10px] font-black text-white uppercase tracking-wider select-none">
                {artigo.autor.substring(0,2).toUpperCase()}
            </div>
